...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    optimization and binary cross-entropy loss.All layers should use a relu
    activation except for the mean and log variance layers in the encoder,
    which should use None,
    ...
    for i in range(1, len(hidden_layers)):
        h_l = keras.layers.Dense(units=hidden_layers[i], activation='relu')
        Y_prev = h_l(Y_prev)
    ...
    h_l_dec = keras.layers.Dense(units=hidden_layers[-1], activation='relu')
    Y_prev = h_l_dec(x_decode)
    for j in range(len(hidden_layers) - 2, -1, -1):
        h_l_dec = keras.layers.Dense(units=hidden_layers[j], activation='relu')
        Y_prev = h_l_dec(Y_prev)
