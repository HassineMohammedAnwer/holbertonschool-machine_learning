...
def lenet5(x, y):
    ...
    y is a tf.placeholder(m, 10):one-hot labels for the network
    ...
    All layers requiring initialization should init their kernels wth he_normal
    __initialization method: tf.keras.initializers.VarianceScaling(scale=2.0)
    All hidden layers requiring activation should use relu activation fun
    you may import tensorflow.compat.v1 as tf
    you may NOT use tf.keras only for the he_normal method.
    Returns:
    a tensor for the softmax activated output
    training op utilizes Adam optimization (with default hyperparameters)
